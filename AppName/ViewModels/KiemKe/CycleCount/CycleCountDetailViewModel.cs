...
namespace AppName
{
    public class CycleCountDetailViewModel : BaseViewModel
    {
        ...
        public CycleCountDetailViewModel(INavigation navigation, StockCountModel itemModel)
        {
            ...
            IsDuplicate = RealmHelper.Instance.All<LocalPermissionModel>().ToArray().Any(q => q.KeyPermission == "DuplicateStockCount"
           && !string.IsNullOrWhiteSpace(q.Role) && q.Role.Contains("VIEW"));

            IsSendAdj = RealmHelper.Instance.All<LocalPermissionModel>().ToArray().Any(q => q.KeyPermission == "SendCycleCount"
           && !string.IsNullOrWhiteSpace(q.Role) && q.Role.Contains("VIEW"));

            IsSendReport = RealmHelper.Instance.All<LocalPermissionModel>().ToArray().Any(q => q.KeyPermission == "SendReportCycelCount"
               && !string.IsNullOrWhiteSpace(q.Role) && q.Role.Contains("VIEW"));
        }

        public partial class SendCycleCountModel
        {
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/stock/SendCycleCount", string.Empty));
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                var dataList = JsonConvert.DeserializeObject<SendCycleCountModel>(content);
        ...
        private async void SendFileCommandASync(object obj)
        {
            try
            {
                ...
                Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/stock/SendCycleCountReport", string.Empty));
