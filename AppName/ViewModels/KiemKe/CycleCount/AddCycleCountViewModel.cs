...
namespace AppName
{
    public class AddCycleCountViewModel : BaseViewModel
    {
        ...
        public AddCycleCountViewModel(INavigation navigation)
        {
        ...
        private async void ConfirmAsync(object obj)
        {
            ...
            try
            {
                ...
                if (ListStockLine.Count() <= 0)
                {
                    var dialog = new NotificationWarrningPopup(this.GetType().Name, CheckConnectInternet.GetmethodName()) { Message = "Stock Lines list data missing." };
                    PopupNavigation.Instance.PushAsync(dialog);
                ...
                var listLine = new List<LSRetail_StockCountDetailModel>();
                foreach (var item in ListStockLine)
                {
                    var line = new LSRetail_StockCountDetailModel();
                    line.Quantity = item.Quantity;
                    line.DateCreate = DateTime.Now;
                    line.UserCreate = item.UserCreate;
                    line.ItemNo = item.ItemNo;
                    line.Zone = 0;
                    line.POG = item.POG;
                    line.FixelID = item.FixID;
                    line.Quantity = item.Quantity;
                    line.Quantity_Scan = item.Quantity_Scan;
                    line.IsDelete = 0;
                    line.BarcodeNo = item.BarcodeNo;
                    line.ItemName = item.ItemName;
                    line.DocumentNo = "";
                    listLine.Add(line);
                }

                model.Lines = listLine;
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (dataList != null)
                    {
                        ...
                        {
                            var dialog = new NotificationWarrningPopup(this.GetType().Name, CheckConnectInternet.GetmethodName()) { Message = "Create a successfuly check sheet." };
                            PopupNavigation.Instance.PushAsync(dialog);
