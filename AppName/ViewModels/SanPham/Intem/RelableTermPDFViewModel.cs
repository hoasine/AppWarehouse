...
namespace AppName
{
    public class RelableTermPDFMultiViewModel : BaseViewModel
    {
        ...
        public RelableTermPDFMultiViewModel(INavigation navigation)
        {
        ...
        private async void PrinterTagPromotion()
        {
            ...
            else
            {
                ...
                try
                {
                    Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/products/ProductInfoPrintTagPDFStreamMulti"));
        ...
        private async void PrinterTag()
        {
            ...
            else
            {
                ...
                try
                {
                    Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/products/ProductInfoPrintTagPDFNotPromotionStreamMulti"));
        ...
        protected async void LoadDataAll()
        {
            try
            {
                var itemTam = RealmHelper.GetItemPromotionAll().Select(item => new ItemPromotionDataModel()
                {
                    ID = item.ID,
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            {
                                realm.RemoveAll<ItemPromotionModel>();
                                transaction.Commit();
                            }
        ...
        protected async void DeleteItemAsync(ItemPromotionDataModel model)
        {
            try
            {
                ...
                var dataCheck = RealmHelper.Instance.All<ItemPromotionModel>().Where(s => s.Barcode == model.Barcode).FirstOrDefault();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (item != null)
                        {
                            //var itemTam = RealmHelper.GetItemPromotion(KeyValue);
                            //if (itemTam == null)
                            //{
                            var listItemUpdate = new ItemPromotionModel
                            {
