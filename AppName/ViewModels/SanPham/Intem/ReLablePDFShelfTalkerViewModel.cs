...
namespace AppName
{
    public class ReLablePDFShelfTalkerViewModel : BaseViewModel
    {
        ...
        public ReLablePDFShelfTalkerViewModel(INavigation navigation)
        {
        ...
        protected async void LoadDataAll()
        {
            try
            {
                var itemTam = RealmHelper.GetItemPromotionPDShelfTalkerAll().Select(item => new ItemPromotionDataModel()
                {
                    ID = item.ID,
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            {
                                realm.RemoveAll<ItemPromotionPDFShelfTalkerModel>();
                                transaction.Commit();
                            }
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            var dataCheck = RealmHelper.Instance.All<ItemPromotionPDFShelfTalkerModel>().Where(s => s.Barcode == model.Barcode).FirstOrDefault();
                ...
                else
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            //if (itemTam == null)
                            //{
                            var listItemUpdate = new ItemPromotionPDFShelfTalkerModel
                            {
