...
namespace AppName
{
    public class ReLablePDFShelfTalkerMultiViewModel : BaseViewModel
    {
        ...
        public ReLablePDFShelfTalkerMultiViewModel(INavigation navigation)
        {
        ...
        private TemplatePrinterValueModel SettingPrint()
        {
            ...
            var getdData = reaml.All<TemplatePrinterModel>().Where(s => s.TypeTemplate == "ShelfTalkerMulti").FirstOrDefault();
            ...
            if (getdData == null)
            {
                using (var transaction = reaml.BeginWrite())
                {
                    ...
                    model.TypeTemplate = "ShelfTalkerMulti";
                    model.Left = "0.4";
                    model.Top = "0.2";
                    model.Right = "0.3";
                    model.Bottom = "0";

                    resuft.TypeTemplate = "ShelfTalkerMulti";
                    model.Left = "0.4";
                    model.Top = "0.2";
                    model.Right = "0.3";
                    resuft.Bottom = "0";
        ...
        private async void OpenSettingCommandAsync()
        {
            ...
            viewModel.ClosePopup += (send, data) =>
            {
                try
                {
                    ...
                    using (var transaction = reaml.BeginWrite())
                    {
                        var model = reaml.All<TemplatePrinterModel>().Where(s => s.TypeTemplate == "ShelfTalkerMulti").FirstOrDefault();
        ...
        private async void PrinterTag()
        {
            ...
            {
                var dialog = new NotificationPopup(this.GetType().Name, CheckConnectInternet.GetmethodName()) {  Message = "No data to print pdf." };
            ...
            else
            {
                ...
                try
                {
                    ...
                    Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/products/ProductInfoPrintTagPDFShelfTalkerNotPromotionStreamMulti"));
        ...
        private async void PrinterTagPromotion()
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/products/ProductInfoPrintTagPDFShelfTalkerStreamMulti"));
        ...
        public RelableTermPDFSmallViewModel.TemplatePrinterValueModel SettingPrintValue { get; private set; }
        ...
        protected async void LoadDataAll()
        {
            try
            {
                var itemTam = RealmHelper.GetItemPromotionPDShelfTalkerMultiAll().Select(item => new ItemPromotionDataModel()
                {
                    ID = item.ID,
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            {
                                realm.RemoveAll<ItemPromotionPDFShelfTalkerMultiModel>();
                                transaction.Commit();
                            }
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            var dataCheck = RealmHelper.Instance.All<ItemPromotionPDFShelfTalkerMultiModel>().Where(s => s.Barcode == model.Barcode).FirstOrDefault();
                            ...
                            var dialog = new NotificationPopup(this.GetType().Name, CheckConnectInternet.GetmethodName()) {  Message = "Delete Items:=" + model.Barcode + " sucessfully." };
                            PopupNavigation.Instance.PushAsync(dialog, false);
                        }
                    }
                    catch (Exception exx)
                ...
                else
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            //if (itemTam == null)
                            //{
                            var listItemUpdate = new ItemPromotionPDFShelfTalkerMultiModel
                            {
