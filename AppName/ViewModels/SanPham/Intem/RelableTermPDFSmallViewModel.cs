...
namespace AppName
{
    public class RelableTermPDFSmallViewModel : BaseViewModel
    {
        ...
        public class TemplatePrinterValueModel
        {
        ...
        public RelableTermPDFSmallViewModel(INavigation navigation)
        {
        ...
        protected async void LoadDataAll()
        {
            try
            {
                var itemTam = RealmHelper.GetItemPromotionPDFSmallAll().Select(item => new ItemPromotionDataModel()
                {
                    ID = item.ID,
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            {
                                realm.RemoveAll<ItemPromotionPDFSmallModel>();
                                transaction.Commit();
                            }
        ...
        protected async void DeleteItemAsync(ItemPromotionDataModel model)
        {
            try
            {
                ...
                var dataCheck = RealmHelper.Instance.All<ItemPromotionPDFSmallModel>().Where(s => s.Barcode == model.Barcode).FirstOrDefault();
        ...
        public static implicit operator RelableTermPDFSmallViewModel(RelableTermPDFSmallWithPOGViewModel v)
        {
