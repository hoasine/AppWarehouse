...
namespace AppName
{
    public class RelableTermPDFViewModel : BaseViewModel
    {
        ...
        public RelableTermPDFViewModel(INavigation navigation)
        {
        ...
        private async void PrinterTagPromotion()
        {
            ...
            else
            {
                ...
                try
                {
                    Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/products/ProductInfoPrintTagPDFStream"));
        ...
        private async void PrinterTag()
        {
            ...
            else
            {
                ...
                try
                {
                    Uri uri = new Uri(string.Format(RealmHelper.Instance.All<UserLSRetailConfig>().FirstOrDefault().URLApi + "/api/products/ProductInfoPrintTagPDFStreamNotPromotion"));
        ...
        protected async void LoadDataAll()
        {
            try
            {
                var itemTam = RealmHelper.GetItemPromotionMultiAll().Select(item => new ItemPromotionDataModel()
                {
                    ID = item.ID,
                ...
                viewModel.ClosePopup += async (send, data) =>
                {
                    try
                    {
                        if (data == true)
                        {
                            ...
                            {
                                realm.RemoveAll<ItemPromotionMultiModel>();
                                transaction.Commit();
                            }
        ...
        protected async void DeleteItemAsync(ItemPromotionDataModel model)
        {
            try
            {
                ...
                var dataCheck = RealmHelper.Instance.All<ItemPromotionMultiModel>().Where(s => s.Barcode == model.Barcode).FirstOrDefault();
                ...
                else
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            //if (itemTam == null)
                            //{
                            var listItemUpdate = new ItemPromotionMultiModel
                            {
